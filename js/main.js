  ...
  solutions: {
    ...
    'LC0-Gen-5': {
      ...
      workflow: [
        ...
        '先準備耐溶劑材質 (例如 金屬, 玻璃) 薄片的周圍隔離罩，把要拆除的組件隔離起來；再將高黏度的 <strong>LC0-Gen-5 半透明防滴膏</strong> 精準塗佈在需要解除的目標膠體上。',
        '做好周圍不耐化學溶劑材質的物理隔絕保護。',
        '<strong>蓋緊保護蓋</strong>，置於 <strong>80°C ~ 100°C</strong> 的烘箱中烘烤 <strong>24 至 96 小時</strong>。',
        '待局部膠體剝離弱化後撬出，先使用 <strong>L0-Gen-1 專用溶劑</strong> 清洗數次；再以 <strong>Gen-0 專用溶劑</strong> 清洗，並於適當溫度（依周遭材質耐溫限制）烘乾。'
      ],
...
const SCIENCE_ARTICLES = {
  'agri-photosynthesis': {
    ...
    content: `
      ...
      <h3>實證效果</h3>
      <p>經過多個農業試驗場實測，在連續噴灑丰泰光合作用促進劑 14-21 天後，鳳梨與芒果的葉綠素相對含量（SPAD值）提升達 10%，單株產量平均增長 10% 至 15%，且果實果糖與維生素C含量有顯著的增長，完美體現了化學材料科技在現代永續農業上的綠色賦能。</p>
    `
  },
  'material-debonding': {
    ...
    content: `
      ...
      <h3>2. 丰泰解膠劑的「膨潤-溶脹」協同機制</h3>
      <p>丰泰技研開發的<strong>「安全無損解UV膠劑與熱固膠劑」</strong>，其運作原理並非暴力溶解，而是溫和的網狀結構分子鏈的滲透、膨潤與溶脹：
      <ul>
      ...
      <p>丰泰全系列解膠劑秉持<strong>綠色化學 (Green Chemistry)</strong> 設計原則，100% 排除氯系（三氯甲烷等）、氟系等鹵素物質與有害芳香烴類溶劑，VOC揮發性極低，對作業人員肺部與皮膚等極度友善，廢液處理符合環保法規，真正實現電子重工與永續循環經濟的共生共榮。</p>
    `
  },
  'agri-spectral': {
    ...
    content: `
      <p>你是否買過外表飽滿紅潤，切開來內部卻早已發黑腐爛的蘋果，或毫無甜度的西瓜？在傳統農業中，蔬果品質的篩選高度依賴人工外觀觀測或隨機破壞性抽樣（榨汁測糖度）。丰泰技研研發的<strong>「手持式蔬果品質影像分析儀」</strong>，透過可攜式多光譜成像技術與 AI 深度學習，為使用者提供了一雙隨身攜帶的「透視眼」。</p>
      <h3>1. 近紅外光譜（NIRS）的無損穿透</h3>
      <p>不同化學分子對不同波長的光線具有獨特的吸收與反射指紋。例如，水果中的果糖、葡萄糖分子含有大量的 O-H 鍵與 C-H 鍵，這些化學鍵在近紅外光波段（700nm - 1100nm）會產生特定共振吸收峰。丰泰影像分析儀發射安全且高密度的光譜光線穿透蔬果果皮，收集反射或透射回來的光子數據，無須切開果實，便能在數秒內計算出糖度（Brix值）、酸度與水分含量。</p>
      <h3>2. 多光譜成像與空間特徵建模</h3>
      <p>單點近紅外光譜只能測量水果的某一個局部，若病變發生在背光側便會漏檢。丰泰手持式分析儀內建精密的多光譜感測模組，當手持探頭貼合果實表面或置於檢測艙時，系統能瞬間捕捉多個特定波段的濾光畫面。AI 演算法將這些光譜圖像特徵拼接重構，建立起果實的光譜特徵模型，無論是內部的褐化、水傷（Watercore）、空心，或是表皮極細微的機械擦傷與黴菌感染，均無所遁形。</p>
      <h3>3. 輕量化邊緣計算與 AI 隨身檢測</h3>
      <p>作為一款手持式設備，其運算效率與耗能控制至關重要。丰泰手持式影像分析儀內部搭載低功耗的高效能邊緣運算模組，可直接在離線或野外環境下，運算經數百萬張樣本訓練的 AI 卷積神經網路（CNN）模型。AI 能在數十毫秒內完成影像分割、光譜特徵提取與品質分析，並即時在手持螢幕上顯示品質分級，協助人員在第一時間做出關鍵決策，非生產線上大型分選裝置。</p>
      <h3>未來展望：賦能田野與供應鏈端的智慧決策</h3>
      <p>這款手持式影像分析儀不僅是一台檢測儀器，更是移動式的農業數據採集終端。主要客群針對農作物田野調查者、小型農戶、採購商、通路商、量販店採購、農會品保、蔬果市場拍賣官以及海關檢驗員。透過採集各產地、各批次的蔬果光譜數據，協助各環節人員即時掌握品質、預估冷鏈保鮮壽命，全面推動臺灣農業與供應鏈走向數據化與高值化的新農業時代。</p>
...
const PRODUCT_DETAIL_DATA = {
  'agri-analyzer': {
    ...
    desc: '本產品目前正處於先端研發與測試階段。為保護關鍵技術智慧財產權與合作社測試數據，相關技術規格與田間量測案例內容暫不對外公開。正式上市時間與詳細規格將於近期公佈，敬請期待！',
    specs: [
      { label: '當前狀態', value: '先端研發與校準測試中' },
      { label: '正式發布', value: '預計近期發布，敬請期待' }
    ],
    cases: []
  },
  'photo-promoter': {
    ...
    specs: [
      ...
      { label: '適用作物', value: '鳳梨、芒果等經濟作物' },
      { label: '增產成效', value: '平均產量提升 10% - 15%' }
    ],
    cases: [
      {
        company: '南投名間鄉廣信農園（鳳梨）與屏東枋山區林家果園（芒果）',
        tag: '陰雨逆境增產',
        challenge: '鳳梨與芒果在生長期常因連續梅雨季節或陰雨天、散射光弱，導致作物光合作用受阻，植株弱化、花芽發育不良，產量大幅下滑。',
        solution: '於生長期及開花著果期，每隔 10 天噴灑一次丰泰「光合作用促進劑肥料」1000倍稀釋液，增強葉綠體在弱光下的光子捕捉效率並活化固碳暗反應。',
        result: '鳳梨/芒果葉片 SPAD 值（葉綠素含量）提升 10%；在低照度陰雨環境下，單株產量增長 10% - 15%，果實糖度平均提高 1.5 度，且提早 5 天採收上市，搶佔高價市場。'
      }
    ]
  },
  'anti-mold': {
    ...
    desc: '採用食品級天然物保護劑，在鳳梨蒂頭切口形成微/奈米級保護阻菌半透膜，阻絕灰黴菌與青黴菌侵入，防止切口發霉腐爛。',
    specs: [
      { label: '安全級別', value: '100% 食品級安全無毒' },
      { label: '防霉效率', value: '蒂頭防霉抑制率 ≥ 99.9%' },
      { label: '海運壽命', value: '海運冷鏈下保鮮延長至 14 - 21 天' },
      { label: '使用方式', value: '免洗，不影響食物風味' }
    ],
    cases: [
      {
        company: '屏東枋山與南投名間鳳梨外銷合作社',
        tag: '跨洋外銷防霉',
        challenge: '鳳梨外銷海運過程中，貨櫃內部溫度變化劇烈，高溫高濕的微氣候導致鳳梨蒂頭切口極易感染黑腐病與灰黴菌，抵港後發霉率常高達 20% 以上，重創品牌信譽。',
        solution: '導入丰泰「截切抗黴保護劑」，在鳳梨採收切蒂後，將保護劑噴施或塗抹於蒂頭切口處，風乾後形成微/奈米級阻菌保護層。',
        result: '鳳梨跨洋海運抵港後的發霉率由 20% 以上大幅降低至 1.5% 以下，且果實完全無任何化學藥劑味道，大幅提升通關合格率與外銷貨架壽命，年減少折損損失數百萬台幣。'
      }
    ]
  },
  'uv-solvent': {
...
const DEFAULT_FORUM_POSTS = [
  {
    id: 'post-1',
    category: 'measure',
    title: '請問手持式多光譜分析儀的甜度檢測精度受哪些因素影響？',
    body: '我們合作社最近想導入手持式多光譜分析儀。想請教各位專家，不同品種的鳳梨（例如金鑽與牛奶鳳梨，或愛文與金煌芒果）其表皮厚度與顏色差異，會不會大幅影響近紅外光譜測量甜度的精準度？校準上需要針對品種做個別的數據建模嗎？',
    ...
    replies: [
      {
        ...
        body: '您好！不同品種的水果由於表皮蠟質層、細胞密度與色素不同，近紅外穿透光譜的散射係數確實有差。丰泰的手持式多光譜分析儀在出廠時，內部已針對主流品種（如金鑽鳳梨、愛文芒果等）建立了多變量校正模型（PLS/ANN）。如果是較為罕見或地方特色品種，系統也提供快速學習模式，只需破壞性抽樣 50 顆果實測量並輸入數值，AI 即可自動微調模型。',
  ...
  },
  {
    ...
    body: '我是台南芒果農，最近生長期常遇到連續一個禮拜以上都陰雨綿綿、沒有陽光的情況。我的果園有些照度不足，想施用促進劑，請問這類逆境下，噴施次數與間隔應該如何安排？',
    author: 'farmer_lin@台南.tw',
    ...
    replies: [
      {
        author: 'tongsimiao@fort-instru.com',
        body: '林先生您好！在連續低溫陰雨的極端逆境下，作物氣孔開放程度較小，吸收率較低。建議將稀釋倍數稍微提高（如 800 倍），並每隔 10 天噴施一次。噴施時請務必選擇在上午 9:00 - 11:00（此時植物生理活性相對稍高）。施用後若能在 4 小時內保持葉面不淋雨，即可發揮最大捕光刺激效果，增強芒果與鳳梨的碳固定能力。',
